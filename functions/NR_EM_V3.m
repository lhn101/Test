function [optim]=NR_EM_V3(data,model,option)
...
parameter_init=model.parameter;
...
while and(search_loop<=option.iteration_limit_calibration, time_loop<(option.time_limit_calibration*60))
    ...
    idx_initParam=parameter_search_idx(parameter_ref(parameter_search_idx)<1E-15);
    parameter(idx_initParam)=1E-10;           % <1E-15 the computational accurancy will be an issue
                                              % Also allow speeding up the EM_NR algorithm
    parameter_ref(idx_initParam)=1E-10;
    ...
    while H_test                               % loop until the hessian can be calculated
        ...
        parfor i=1:2
            [~,~,~,~,log_lik_s(i),~,~]=SKF(data,model_store{i},option); % LL calcultation
        end
    ...
    loop_ll=0;
    trial_delta_TR=zeros(1,2*nb_levels_lambda_ref);
    trial_log_lik=zeros(1,2*nb_levels_lambda_ref);
    trial_paramTR_loop=zeros(1,2*nb_levels_lambda_ref);
    trial_paramOR_loop=zeros(1,2*nb_levels_lambda_ref);
    trial_parameter=zeros(size(parameter,1),2*nb_levels_lambda_ref);
    trial_model_store=cell(1,2*nb_levels_lambda_ref);
    trial_model=model;
    trial=0;  
    while loop_converged 
        ...
        if log_lik_test>log_lik_0
            ...
            % Parallel computing
            if ~trial
                for n=1:2*nb_levels_lambda_ref
                    if n==1
                        trial_delta_TR(n)=delta_p_TR/(2^n);
                    else
                        trial_delta_TR(n)=trial_delta_TR(n-1)/(2^n);
                    end
                    
                    if n>nb_levels_lambda_ref
                        if n==nb_levels_lambda_ref+1
                            trial_delta_TR(n)=-delta_p_TR/(2^(n-nb_levels_lambda_ref));
                        else
                            trial_delta_TR(n)=trial_delta_TR(n-1)/(2^(n-nb_levels_lambda_ref));
                        end
                    end  
                    
                    trial_paramTR_loop(n)=param_TR+trial_delta_TR(n);
                    trial_paramOR_loop(n)=fct_inv_TR(trial_paramTR_loop(n));
                    
                    trial_parameter(:,n)=parameter_ref;
                    trial_parameter(param_idx_loop,n)=trial_paramOR_loop(n);
                    trial_model.parameter=trial_parameter(:,n);
                    trial_model_store{n}=trial_model;
                end
                
                parfor l=1:2*nb_levels_lambda_ref
                    try
                        %% LL test
                        [~,~,~,~,trial_log_lik(l),~,~]=SKF(data,trial_model_store{l},option);
                    catch err 
                        trial_log_lik(l)=-inf;
                    end
                end
                
                [log_lik_test,max_idx]=max(trial_log_lik);
                
                parameter(param_idx_loop)=trial_paramOR_loop(max_idx);
                parameter_TR(param_idx_loop)=trial_paramTR_loop(max_idx);
                trial=1;
                
                delta_p=trial_paramOR_loop(max_idx)-parameter_ref(param_idx_loop);
                if max_idx>nb_levels_lambda_ref
                    idx_n=max_idx-nb_levels_lambda_ref;
                else
                    idx_n=max_idx;
                end
                if disp_flag==1
                    disp(['           Warning: log-likelihood has decreased -> delta_param: ' sprintf('%#-+8.2e',delta_p) ' (delta_p_TR=delta_p_TR/(2^' num2str(idx_n) '))'])
                end
            else
                if disp_flag==1
                    disp(' ')
