function [x, V, VV, S] = RTS_SKS(estim,data,model)

T = size(estim.S,1); %Number of time stamps
...
for t=T-1:-1:1
    ...
    U = zeros(M,M);
    for k=1:M
        A_k = model.A{k}(model.parameter(model.p_ref),data.timestamps(t+1),data.dt_steps(t+1));
        Z_k = model.Z(model.parameter(model.p_ref),data.timestamps(t+1),data.dt_steps(t+1));
        for j=1:M
            Q_k = model.Q{k}{j}(model.parameter(model.p_ref),data.timestamps(t+1),data.dt_steps(t+1));

            [x_jk{j}(:,k), V_jk{j}(:,:,k), VV_jk{k}(:,:,j)] = smooth_update_SKF(x{k}(:,t+1), V{k}(:,:,t+1), estim.x_M{j}(:,t), estim.V_M{j}(:,:,t), estim.V_M{k}(:,:,t+1), estim.VV_M{k}(:,:,t+1), A_k, Q_k);
            U(j,k)=estim.S(t,j)*Z_k(j,k);
        end
        U(:,k)=U(:,k)/sum(U(:,k));
        for j=1:M
