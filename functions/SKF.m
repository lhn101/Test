function [x, V, VV, S, loglik,U,D] = SKF(data,model,option)
% INPUTS:
...
%                 Q{j,i,t}(2,2)=model.parameter(sigma22_idx)^2*(data.dt_steps(t)^2/data.dt_ref);
%             elseif and(j==2,i==1)
%                 QQ = model.Q{j}{j}(model.parameter(model.p_ref),data.timestamps(t),data.dt_steps(t));
%                 Q{j,i,t}=diag(diag(QQ));
%                 Q{j,i,t}(2,2)=model.parameter(sigma22_idx)^2*(data.dt_steps(t)^4/(3*data.dt_ref));
%             end
            ...
            if strcmp(option.method,'UD')
                %% UD filter
                [x_ij{j}(:,i), V_ij{j}(:,:,i), VV_ij{j}(:,:,i), U{i,j,t+1}, D{i,j,t+1}, LL(i,j,t)] = UDF(A{j,t},C{j,t},Q{j,i,t},R{j,t}, data.values(t,:)', prevX, prevV, U{i,j,t}, D{i,j,t});
            else
                %% Kalman filter
                warning('off','all')
                [x_ij{j}(:,i), V_ij{j}(:,:,i), VV_ij{j}(:,:,i), LL(i,j,t)] = KF(A{j,t},C{j,t},Q{j,i,t},R{j,t}, data.values(t,:)', prevX, prevV);
                warning('on','all')
            end
